...
Changes:
    2021.11.20  deep constant propagation ported from CSeq 2.1 [SV-COMP 2022]
	...
	def _evaluate_sizeof(self, n, modifiers=[]):
		...
		if typ == pycparser.c_ast.ID:
			...
			# e.g., sizeof(varname) or sizeof(struct acjfdh)
				...
				while (block,n.name) not in self.Parser.vars and block != '0':
					block = block[:block.rfind('.')]

				if (block,n.name) in self.Parser.vars:
					w = self._evaluate_sizeof(self.Parser.vars[block,n.name].type)
			...
			if t in self.bytewidth:
				...
				while (block,t) not in self.Parser.typedefs and block != '0':
					block = block[:block.rfind('.')]

				if (block,t) in self.Parser.typedefs:
					t = self.Parser.typedefs[block,t].type
					w = self._evaluate_sizeof(t)

		elif typ == pycparser.c_ast.Struct:
			block = self.blockid

			while (block,n.name) not in self.Parser.structs and block != '0':
				block = block[:block.rfind('.')]

			if (block,n.name) in self.Parser.structs:
				s = self.Parser.structs[block,n.name].decls
				cnt = 0

				if s is not None:
					for f in s:
						##cnt += self._evaluate_sizeof(f)
						fieldsize = self._evaluate_sizeof(f)
						if fieldsize is not None: cnt+=fieldsize
						else:
							cnt = None
							break
				else:
				...
				self.debug("evaluation of sizeof not supported for struct '%s'" % (n.name))
				w = None
