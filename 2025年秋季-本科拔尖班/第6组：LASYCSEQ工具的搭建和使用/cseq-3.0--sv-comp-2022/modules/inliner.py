...
Changes:
    2021.11.20  simplified inlining partly ported from CSeq 2.1 [SV-COMP 2022]
    2021.02.13 (CSeq 3.0)
...
To do:
  - port simplified inlining from CSeq 2.1 [SV-COMP 2021] completely
...
class inliner(core.module.Translator):
	...
	def _inlineFunction(self,fdef,fcall,simple):
		...
		#####	for cnt in range(0,len(fcall.args.exprs)):
		#####		print("function %s param %s" % (fdef.decl.name,fdef.decl.type.args.params[cnt].name))
		if fcall.args is not None:
			for paramno,expr in enumerate(fcall.args.exprs):   # for each parameter in the function call
				self.log("%s: analysing parameter %s for function %s" % (self._mapbacklineno(self.currentinputlineno)[0],paramno,fname))
				...
				if (type(expr) == pycparser.c_ast.ID and
					self.Parser.isglobalvariable(self.blockid,expr.name)):
					self.log("%s: simplified passing of global variable '%s' as argument %s in call to '%s'" % (self._mapbacklineno(self.currentinputlineno)[0],expr.name,paramno,fname))
					paramnochange.append(paramno)
				elif type(expr) == pycparser.c_ast.Constant:
					self.log("%s: simplified passing of constant value '%s' as argument %s in call to '%s'" % (self._mapbacklineno(self.currentinputlineno)[0],expr.value,paramno,fname))
					paramnochange.append(paramno)
