...
class Parser(pycparserext.ext_c_generator.GnuCGenerator): #class Parser(pycparser.c_ast.NodeVisitor):
	...
	def visit_Decl(self,n,no_type=False):
		# The following snippet has beenported from v2.1 [SV-COMP 2021] and enables
		# deep constant propagation.
		# It is independent w.r.t. what happen below, but it should be merged with
		# the rest as soon as possible. TODO
		if isinstance(n.type,pycparser.c_ast.Struct):
			#print ("= - > new structure declared             [%s,%s,%s]  ===============" % (self.blockid,n.type.name,n.type.decls))
			#print ("= - > new structure declared (b)            [%s] " % (n.type.name))
	...
	def visit_Typedef(self,n):
		...
		if isinstance(n.type.type,pycparser.c_ast.Struct):
			if n.type.type.decls:
				...
				#print ("= - > new structure declared (a)            [%s] " % (n.type.type.name))
				self.structs[self.blockid,n.type.type.name] = n.type.type
